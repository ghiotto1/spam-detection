...
# Purpose
The `DefaultScheduleContext` class implements the `ScheduleContextInterface` interface. It is part of the `io.kestra.core.schedulers` package. The class is intended for use in testing scenarios, as indicated by the comment. It provides a specific implementation of the `ScheduleContextInterface` without additional functionality or methods.
