...
# Purpose
The `ScheduleContextInterface` is a Java interface within the `io.kestra.core.schedulers` package. It defines a contract for classes that implement it, but it does not specify any methods or fields. This interface provides narrow functionality as it serves as a marker or a placeholder for future development or for use in type hierarchies. It can be used to identify or group classes that share a common characteristic related to scheduling contexts within the application.
