...
# Purpose
This code implements a chess game engine in C. It provides functions to create and manage a chess game, initialize game states from a FEN (Forsyth-Edwards Notation) string, and perform operations such as playing moves, undoing moves, and checking game status. The code defines a `chess` structure that maintains the state of the game, including the board history, move history, current legal moves, repetition count, and terminal state. The functions in the code allow for the creation of a new chess game, either from the initial position or a specified FEN string, and manage the memory associated with the game state.

The code includes functions to retrieve various aspects of the game state, such as the current board, legal moves, terminal state, and move history. It also provides functionality to check if a player is in check, if a square is attacked, and if certain draw conditions can be claimed, such as the 50-move rule or threefold repetition. The [`chessCalculateFields`](<#chesscalculatefields>) function updates the game state by recalculating legal moves, checking for checkmate or stalemate, and determining if draw conditions are met. The code relies on external components, such as `board` and `move` structures, which are included from the `chesslib` library, indicating that this file is part of a larger chess library or application.
...
- **Logic and Control Flow**:
    - Calls the function [`chessInitFenInPlace`](<#chessinitfeninplace>) with the `chess` structure `c` and the constant `INITIAL_FEN` to set up the initial chess position.
...
Retrieves the list of current legal moves for a given chess game state.
- **Inputs**:
    - `c`: A pointer to a `chess` structure representing the current state of the chess game.
- **Logic and Control Flow**:
...
Executes a move in a chess game if the move is legal and the game is ongoing.
- **Inputs**:
    - `c`: A pointer to a `chess` structure representing the current state of the chess game.
...
- **Logic and Control Flow**:
    ...
    - Recalculate the game fields by calling [`chessCalculateFields`](<#chesscalculatefields>).
...
- **Output**: Returns 0 on success, indicating the move was undone, or 1 on failure, indicating no move was undone.
- **Functions Called**:
...
- **Logic and Control Flow**:
    ...
    - Accesses the `castleState` field of the board structure returned by [`chessGetBoard`](<#chessgetboard>).
...
- **Inputs**:
    - `c`: A pointer to a `chess` structure representing the current state of the chess game.
- **Logic and Control Flow**:
    - Calls the function [`chessGetBoard`](<#chessgetboard>) with the input `c` to get the current board state.
    - Accesses the `moveNumber` field from the returned board structure.
...
- **Inputs**:
    - `c`: A pointer to a `chess` structure representing the current state of the chess game.
- **Logic and Control Flow**:
    - Calls [`chessGetMoveHistory`](<#chessgetmovehistory>) with the `chess` pointer `c` to retrieve the move history of the game.
    - Passes the retrieved move history to [`moveListGetUciString`](<movelist.c.md#movelistgetucistring>) to convert it into a UCI formatted string.
    - Returns the UCI formatted string.
...
- **Inputs**:
    - `c`: A pointer to a `chess` structure representing the current state of the chess game.
- **Logic and Control Flow**:
    - Calls [`chessGetBoard`](<#chessgetboard>) to retrieve the current board from the `chess` structure `c`.
    - Passes the retrieved board to the [`boardIsInCheck`](<board.c.md#boardisincheck>) function to determine if the king is in check.
    - Returns the result from [`boardIsInCheck`](<board.c.md#boardisincheck>).
...
Checks if a specific square on the chess board is attacked by the opponent's pieces.
- **Inputs**:
    - `c`: A pointer to a `chess` structure representing the current state of the chess game.
...
- **Logic and Control Flow**:
    ...
    - Determine the opponent's color by checking the current player's color on the board and setting the opponent to the opposite color.
...
Determines if a chess game can claim a draw based on the 50-move rule.
- **Inputs**:
    - `c`: A pointer to a `chess` structure representing the current state of the chess game.
- **Logic and Control Flow**:
...
Claims a draw in a chess game if the 50-move rule condition is met.
- **Inputs**:
    - `c`: A pointer to a `chess` structure representing the current state of the chess game.
- **Logic and Control Flow**:
    - Checks if a draw can be claimed using the 50-move rule by calling [`chessCanClaimDraw50`](<#chesscanclaimdraw50>) with the chess game state `c`.
    - If [`chessCanClaimDraw50`](<#chesscanclaimdraw50>) returns true, sets the `terminal` state of the chess game to `tsDrawClaimed50MoveRule`.
- **Output**: No return value; modifies the `terminal` state of the `chess` structure if the draw is claimed.
- **Functions Called**:
...
- **Inputs**:
    - `c`: A pointer to a `chess` structure representing the current state of the chess game.
- **Logic and Control Flow**:
    - Checks if a draw can be claimed due to threefold repetition by calling [`chessCanClaimDrawThreefold`](<#chesscanclaimdrawthreefold>) with the `chess` structure `c`.
    - If [`chessCanClaimDrawThreefold`](<#chesscanclaimdrawthreefold>) returns true, sets the `terminal` field of the `chess` structure `c` to `tsDrawClaimedThreefold`.
- **Output**: No return value; modifies the `terminal` field of the `chess` structure `c` if a draw is claimed.
- **Functions Called**:
...
- **Logic and Control Flow**:
    ...
    - Generate the list of legal moves for the current board and assign it to `c->currentLegalMoves`.
    ...
    - If there are legal moves, check for draw conditions such as fivefold repetition, 75-move rule, insufficient material, or ongoing state.
