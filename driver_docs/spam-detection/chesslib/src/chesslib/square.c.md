...
# Purpose
This code provides functionality for handling chessboard squares. It defines several functions to create and manipulate squares on a chessboard, using both integer coordinates and Standard Algebraic Notation (SAN). The function [`sqI`](<#sqi>) creates a square from integer file and rank values, while [`sqS`](<#sqs>) creates a square from a SAN string. The code also includes a constant array `SQ_STRS` that holds SAN strings for all 64 squares on a chessboard.

Additional functions include [`sqIndex`](<#sqindex>), which creates a square from an index, and [`sqGetIndex`](<#sqgetindex>), which returns the index of a given square. The function [`sqGetStr`](<#sqgetstr>) retrieves the SAN string for a square, and [`sqIsDark`](<#sqisdark>) determines if a square is dark-colored. The function [`sqEq`](<#sqeq>) checks if two squares are equal. The code is intended to be part of a larger library, as indicated by the inclusion of the header file `chesslib/square.h`. It does not define public APIs or external interfaces directly but provides utility functions for chessboard square operations.
...
- **Logic and Control Flow**:
    ...
    - If the index is valid, return the corresponding SAN string from the `SQ_STRS` array.
...
Compares two chessboard squares for equality, marking them as invalid if they are out of bounds.
- **Inputs**:
...
- **Logic and Control Flow**:
    - Check if `s1` is out of bounds by verifying if its `file` or `rank` is less than 1 or greater than 8; if so, set `s1` to `SQ_INVALID`.
    - Check if `s2` is out of bounds by verifying if its `file` or `rank` is less than 1 or greater than 8; if so, set `s2` to `SQ_INVALID`.
